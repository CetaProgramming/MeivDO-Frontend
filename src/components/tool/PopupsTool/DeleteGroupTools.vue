...
export default {
    ...
    computed: {
        langs() {
            return langStore().getLang.PageTool.PopupDeleteGroupTool
        }
    },
    methods: {
        async deleteTool() {
            try {
                await groupToolsStore().deleteTool(this.value)
                this.$emit("closePopUp");
                this.$emit("activeToast", {
                    msg: this.langs.DeleteGroupToolSucess,
                    type: markRaw(ToastSuccess)
                });
            } catch (error) {
                this.$emit("activeToast", {
                    msg: this.langs.DeleteGroupToolError,
